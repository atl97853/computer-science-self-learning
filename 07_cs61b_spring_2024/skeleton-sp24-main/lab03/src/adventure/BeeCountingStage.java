...
public class BeeCountingStage implements AdventureStage {
    ...
    public BeeCountingStage(In in) {
        ...
        this.input = new ArrayList<>(); // When and object is declared, but not initialized, it is null.
    }
    ...
    private int sumInput() {
        int sum = 0;
        for (int i = 0; i <= this.input.size() - 1; i++) { // Fix bug, IndexOutOfBoundsException, this.input.size() - 1.
